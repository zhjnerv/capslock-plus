getSystemLanguage(){
    ...
    if(systemLocale)
    {
        systemLocale := SubStr(systemLocale, -4) ; Fix: Grab last 4 digits to match Map keys
    }

    if(systemLocale && languageCodes.Has(systemLocale))
    {
        return languageCodes[systemLocale]
    } else {
        ; A_Language In v2 is 4-digit hex string usually?
        ; V2 A_Language returns the system default language ID.
        return languageCodes.Has(A_Language) ? languageCodes[A_Language] : ""
    }
}
